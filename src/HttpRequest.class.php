	...
	*/
	public function cookiejar($filepath) {
		if(is_file($filepath) === false && is_string($filepath) === false) {
			throw new Exception("HttpRequest::cookiejar Expects filepath to be of type file. ".gettype($filepath)." was given.");
		} else if(fopen($filepath, "w+") === false) {
			throw new Exception("Unable to create new cookiejar '".$filepath."'");
		}
